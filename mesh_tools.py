...
def filter_edge(mesh, edge_ccs, params, invalid=False):
    context_ccs = [set() for _ in edge_ccs]
    mesh_nodes = mesh.nodes
    for edge_id, edge_cc in enumerate(edge_ccs):
        if params['context_thickness'] == 0:
            continue
        edge_group = {}
...
def extrapolate(global_mesh,
    ...
    if "up" in direc.lower() and "-" not in direc.lower():
        all_anchor = [0, h_off + params['context_thickness'], w_off, w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
        mask_anchor = [0, h_off, w_off, w_off + noext_W]
        context_anchor = [h_off, h_off + params['context_thickness'], w_off, w_off + noext_W]
        ...
        all_anchor = [h_off + noext_H - params['context_thickness'], 2 * h_off + noext_H, w_off, w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
        mask_anchor = [h_off + noext_H, 2 * h_off + noext_H, w_off, w_off + noext_W]
        context_anchor = [h_off + noext_H - params['context_thickness'], h_off + noext_H, w_off, w_off + noext_W]
        valid_line_anchor = [h_off + noext_H - 1, h_off + noext_H, w_off, w_off + noext_W]
        valid_anchor = [min(mask_anchor[0], context_anchor[0]), max(mask_anchor[1], context_anchor[1]),
                        min(mask_anchor[2], context_anchor[2]), max(mask_anchor[3], context_anchor[3])]
    elif "left" in direc.lower() and "-" not in direc.lower():
        all_anchor = [h_off, h_off + noext_H, 0, w_off + params['context_thickness']]
        global_shift = [all_anchor[0], all_anchor[2]]
        mask_anchor = [h_off, h_off + noext_H, 0, w_off]
        context_anchor = [h_off, h_off + noext_H, w_off, w_off + params['context_thickness']]
        valid_line_anchor = [h_off, h_off + noext_H, w_off, w_off + 1]
        valid_anchor = [min(mask_anchor[0], context_anchor[0]), max(mask_anchor[1], context_anchor[1]),
                        min(mask_anchor[2], context_anchor[2]), max(mask_anchor[3], context_anchor[3])]
    elif "right" in direc.lower() and "-" not in direc.lower():
        all_anchor = [h_off, h_off + noext_H, w_off + noext_W - params['context_thickness'], 2 * w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
        mask_anchor = [h_off, h_off + noext_H, w_off + noext_W, 2 * w_off + noext_W]
        context_anchor = [h_off, h_off + noext_H, w_off + noext_W - params['context_thickness'], w_off + noext_W]
        ...
        all_anchor = [0, h_off + params['context_thickness'], 0, w_off + params['context_thickness']]
        global_shift = [all_anchor[0], all_anchor[2]]
        ...
        all_anchor = [h_off + noext_H - params['context_thickness'], 2 * h_off + noext_H, 0, w_off + params['context_thickness']]
        global_shift = [all_anchor[0], all_anchor[2]]
        ...
        all_anchor = [0, h_off + params['context_thickness'], w_off + noext_W - params['context_thickness'], 2 * w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
        ...
        all_anchor = [h_off + noext_H - params['context_thickness'], 2 * h_off + noext_H, w_off + noext_W - params['context_thickness'], 2 * w_off + noext_W]
        global_shift = [all_anchor[0], all_anchor[2]]
    ...
    depth_edge_output = depth_edge_model.forward_3P(t_mask, t_context, t_rgb, t_disp, t_edge, unit_length=128,
                                                    cuda=device)
    t_output_edge = (depth_edge_output> params['ext_edge_threshold']).float() * t_mask + t_edge
    ...
    # rgb_output = rgb_feat_model.forward_3P(t_mask, t_context, t_rgb, t_update_edge, unit_length=128, cuda=params['gpu_ids'])
...
def fill_dummy_bord(mesh, info_on_pix, image, depth, params):
...
def enlarge_border(mesh, info_on_pix, depth, image, params):
    mesh.graph['hoffset'], mesh.graph['woffset'] = params['extrapolation_thickness'], params['extrapolation_thickness']
    ...
    #                               (params['extrapolation_thickness'], params['extrapolation_thickness']), (0, 0)),
    #                    mode='constant')
...
def edge_inpainting(edge_id, context_cc, erode_context_cc, mask_cc, edge_cc, extend_edge_cc,
                    mesh, edge_map, edge_maps_with_id, params, union_size, depth_edge_model, inpaint_iter):
    ...
    if require_depth_edge(patch_edge_dict['edge'], patch_edge_dict['mask']) and inpaint_iter == 0:
        with torch.no_grad():
            device = params["gpu_ids"] if isinstance(params["gpu_ids"], int) and params["gpu_ids"] >= 0 else "cpu"
...
def depth_inpainting(context_cc, extend_context_cc, erode_context_cc, mask_cc, mesh, params, union_size, depth_feat_model, edge_output, given_depth_dict=False, spdb=False):
    if given_depth_dict is False:
        depth_dict = get_depth_from_nodes(context_cc | extend_context_cc, erode_context_cc, mask_cc, mesh.graph['H'], mesh.graph['W'], mesh, params['log_depth'])
