...
def sparse_bilateral_filtering(
    depth, image, params, HR=False, mask=None, gsHR=True, edge_id=None, num_iter=None, num_gs_iter=None, spdb=False
):
    """
    params:
    - filter_size
    ...
    for i in range(num_iter):
        if isinstance(params["filter_size"], list):
            window_size = params["filter_size"][i]
        else:
            window_size = params["filter_size"]
        vis_image = image.copy()
        save_images.append(vis_image)
        save_depths.append(vis_depth)
        u_over, b_over, l_over, r_over = vis_depth_discontinuity(vis_depth, params, mask=mask)
        ...
        vis_depth = bilateral_filter(
            vis_depth, params, discontinuity_map=discontinuity_map, HR=HR, mask=mask, window_size=window_size
        )

    return save_images, save_depths


def vis_depth_discontinuity(depth, params, vis_diff=False, label=False, mask=None):
    """
    params:
    - 
    """
...
def bilateral_filter(depth, params, discontinuity_map=None, HR=False, mask=None, window_size=False):
    sort_time = 0
    replace_time = 0
    filter_time = 0
    init_time = 0
