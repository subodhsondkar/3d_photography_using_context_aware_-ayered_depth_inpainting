...
def create_mesh(depth, image, int_mtx, params):
    H, W, C = image.shape
    ext_H, ext_W = H + 2 * params['extrapolation_thickness'], W + 2 * params['extrapolation_thickness']
    ...
    disp = 1. / (-depth)
    LDI.graph['hoffset'], LDI.graph['woffset'] = params['extrapolation_thickness'], params['extrapolation_thickness']
    ...
    image = np.pad(image,
                    pad_width=((params['extrapolation_thickness'], params['extrapolation_thickness']),
                               (params['extrapolation_thickness'], params['extrapolation_thickness']),
                               (0, 0)),
                    mode='constant')
    depth = np.pad(depth,
                    pad_width=((params['extrapolation_thickness'], params['extrapolation_thickness']),
...
def generate_init_node(mesh, params, min_node_in_cc):
    mesh_nodes = mesh.nodes
...
def generate_face(mesh, info_on_pix, params):
    H, W = mesh.graph['H'], mesh.graph['W']
    str_faces = []
    num_node = len(mesh.nodes)
    ply_flag = params.get('save_ply')
...
def group_edges(LDI, params, image, remove_conflict_ordinal, spdb=False):
                ...
                for diag_candi in LDI.neighbors(ne_node):
                    if [diag_candi[0], diag_candi[1]] in diagonal_xys and LDI.degree(diag_candi) < 4:
                        if diag_candi not in inval_diag_candi:
                            if not exceed_thre(1./node[2], 1./diag_candi[2], params['depth_threshold']) or \
...
def remove_redundant_edge(mesh, edge_mesh, edge_ccs, info_on_pix, params, redundant_number=1000, invalid=False, spdb=False):
    point_to_amount = {}
    point_to_id = {}
    ...
    valid_edge_ccs = filter_edge(mesh, edge_ccs, params, invalid=invalid)
...
def remove_dangling(mesh, edge_ccs, edge_mesh, info_on_pix, image, depth, params):
...
def context_and_holes(mesh, edge_ccs, params, specific_edge_id, specific_edge_loc, depth_feat_model,
    ...
    if inpaint_iter == 0:
        background_thickness = params['background_thickness']
        context_thickness = params['context_thickness']
    else:
        background_thickness = params['background_thickness_2']
        context_thickness = params['context_thickness_2']
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
        ...
        if inpaint_iter == 0:
            depth_dict = get_depth_from_maps(context_map, mask_map, context_depth, mesh.graph['H'], mesh.graph['W'], log_depth=params['log_depth'])
            ...
            depth_dict = depth_inpainting(None, None, None, None, mesh, params, union_size, depth_feat_model, None, given_depth_dict=depth_dict, spdb=False)
        ...
        if inpaint_iter == 0:
            tmp_width = params['depth_edge_dilate']
        else:
            tmp_width = 0
...
def DL_inpaint_edge(mesh,
    ...
    if isinstance(params["gpu_ids"], int) and (params["gpu_ids"] >= 0):
        device = params["gpu_ids"]
    else:
        device = "cpu"
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc, edge_cc) in enumerate(zip(context_ccs, mask_ccs, erode_context_ccs, extend_context_ccs, edge_ccs)):
        ...
        if require_depth_edge(patch_edge_dict['edge'], patch_edge_dict['mask']) and inpaint_iter == 0:
            ...
            tensor_edge_dict['output'] = (depth_edge_output> params['ext_edge_threshold']).float() * tensor_edge_dict['mask'] + tensor_edge_dict['edge']
        else:
        ...
        if require_depth_edge(patch_edge_dict['edge'], patch_edge_dict['mask']) and inpaint_iter == 0:
            if ((depth_edge_output> params['ext_edge_threshold']).float() * tensor_edge_dict['mask']).max() > 0:
                try:
                    edge_dict['fpath_map'], edge_dict['npath_map'], break_flag, npaths, fpaths, invalid_edge_id = \
                        clean_far_edge_new(edge_dict['output'], end_depth_maps, edge_dict['mask'], edge_dict['context'], mesh, info_on_pix, edge_dict['self_edge'], inpaint_iter, params)
                except:
                    import pdb; pdb.set_trace()
                pre_npath_map = edge_dict['npath_map'].copy()
                if params.get('repeat_inpaint_edge') is True:
                    for _ in range(2):
                        ...
                        depth_edge_output = depth_edge_model.forward_3P(tensor_edge_dict['mask'],
                                                    ...
                                                    cuda=device)
                        depth_edge_output = depth_edge_output.cpu()
                        depth_edge_output = (depth_edge_output> params['ext_edge_threshold']).float() * tensor_edge_dict['mask'] + tensor_edge_dict['edge']
                        ...
                        full_depth_edge_output[union_size['x_min']:union_size['x_max'], union_size['y_min']:union_size['y_max']] = \
                            depth_edge_output
                        edge_dict['fpath_map'], edge_dict['npath_map'], break_flag, npaths, fpaths, invalid_edge_id = \
                            clean_far_edge_new(full_depth_edge_output, end_depth_maps, edge_dict['mask'], edge_dict['context'], mesh, info_on_pix, edge_dict['self_edge'], inpaint_iter, params)
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc, edge_cc) in enumerate(zip(context_ccs, mask_ccs, erode_context_ccs, extend_context_ccs, edge_ccs)):
        ...
        depth_dict = depth_inpainting(context_cc, extend_context_cc, erode_context_cc | extend_erode_context_ccs[edge_id], mask_cc, mesh, params, union_size, depth_feat_model, edge_dict['output'])
        ...
        init_frac = params['largest_size'] / (np.array([*patch_rgb_dict['mask'].shape[-2:]]).prod() ** 0.5)
        ...
        with torch.no_grad():
            ...
            if params.get('gray_image') is True:
    ...
    if inpaint_iter == 0:
        mesh, info_on_pix = refine_color_around_edge(mesh, info_on_pix, new_edge_ccs, params, False)
...
def write_ply(name,
    ...
    input_mesh, xy2depth, image, depth = create_mesh(depth, image, int_mtx, params)

    H, W = input_mesh.graph['H'], input_mesh.graph['W']
    input_mesh = tear_edges(input_mesh, params['depth_threshold'], xy2depth)
    input_mesh, info_on_pix = generate_init_node(input_mesh, params, min_node_in_cc=200)
    edge_ccs, input_mesh, edge_mesh = group_edges(input_mesh, params, image, remove_conflict_ordinal=False)
    ...
    specific_edge_id = []
    edge_ccs, input_mesh, edge_mesh = group_edges(input_mesh, params, image, remove_conflict_ordinal=True)
    pre_depth = depth.copy()
    input_mesh, info_on_pix, edge_mesh, depth, aft_mark = remove_dangling(input_mesh, edge_ccs, edge_mesh, info_on_pix, image, depth, params)
    ...
    if params['extrapolate_border'] is True:
        pre_depth = depth.copy()
        ...
        input_mesh, info_on_pix, depth, image = enlarge_border(input_mesh, info_on_pix, depth, image, params)
        noext_H, noext_W = H, W
        H, W = image.shape[:2]
        input_mesh, info_on_pix = fill_dummy_bord(input_mesh, info_on_pix, image, depth, params)
        edge_ccs, input_mesh, edge_mesh = \
            group_edges(input_mesh, params, image, remove_conflict_ordinal=True)
        input_mesh = combine_end_node(input_mesh, edge_mesh, edge_ccs, depth)
        input_mesh, depth, info_on_pix = update_status(input_mesh, info_on_pix, depth)
        edge_ccs, input_mesh, edge_mesh = \
            group_edges(input_mesh, params, image, remove_conflict_ordinal=True, spdb=False)
        input_mesh = remove_redundant_edge(input_mesh, edge_mesh, edge_ccs, info_on_pix, params, redundant_number=params['redundant_number'], spdb=False)
        input_mesh, depth, info_on_pix = update_status(input_mesh, info_on_pix, depth)
        edge_ccs, input_mesh, edge_mesh = group_edges(input_mesh, params, image, remove_conflict_ordinal=True)
        input_mesh = combine_end_node(input_mesh, edge_mesh, edge_ccs, depth)
        input_mesh = remove_redundant_edge(input_mesh, edge_mesh, edge_ccs, info_on_pix, params, redundant_number=params['redundant_number'], invalid=True, spdb=False)
        input_mesh, depth, info_on_pix = update_status(input_mesh, info_on_pix, depth)
        edge_ccs, input_mesh, edge_mesh = group_edges(input_mesh, params, image, remove_conflict_ordinal=True)
        input_mesh = combine_end_node(input_mesh, edge_mesh, edge_ccs, depth)
        input_mesh, depth, info_on_pix = update_status(input_mesh, info_on_pix, depth)
        ...
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(input_mesh, info_on_pix, image, depth, other_edge_with_id, edge_map, edge_ccs,
                                                depth_edge_model, depth_feat_model, rgb_model, params, direc="up")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(input_mesh, info_on_pix, image, depth, other_edge_with_id, edge_map, edge_ccs,
                                                depth_edge_model, depth_feat_model, rgb_model, params, direc="left")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(input_mesh, info_on_pix, image, depth, other_edge_with_id, edge_map, edge_ccs,
                                                depth_edge_model, depth_feat_model, rgb_model, params, direc="down")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(input_mesh, info_on_pix, image, depth, other_edge_with_id, edge_map, edge_ccs,
                                                depth_edge_model, depth_feat_model, rgb_model, params, direc="right")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(input_mesh, info_on_pix, image, depth, other_edge_with_id, edge_map, edge_ccs,
                                                depth_edge_model, depth_feat_model, rgb_model, params, direc="right-up")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(input_mesh, info_on_pix, image, depth, other_edge_with_id, edge_map, edge_ccs,
                                                depth_edge_model, depth_feat_model, rgb_model, params, direc="right-down")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(input_mesh, info_on_pix, image, depth, other_edge_with_id, edge_map, edge_ccs,
                                                depth_edge_model, depth_feat_model, rgb_model, params, direc="left-up")
        info_on_pix, input_mesh, image, depth, edge_ccs = extrapolate(input_mesh, info_on_pix, image, depth, other_edge_with_id, edge_map, edge_ccs,
                                                depth_edge_model, depth_feat_model, rgb_model, params, direc="left-down")
    specific_edge_loc = None
    specific_edge_id = []
    vis_edge_id = None
    ...
    vis_edge_ccs = filter_edge(input_mesh, edge_ccs, params)
    ...
    input_mesh, info_on_pix, specific_edge_nodes, new_edge_ccs, connect_points_ccs, image = DL_inpaint_edge(input_mesh,
                                                                                                            ...
                                                                                                            inpaint_iter=0)
    specific_edge_id = []
    if params["run_dl_inpaint_twice"]:
        ...
        context_ccs, mask_ccs, broken_mask_ccs, edge_ccs, erode_context_ccs, init_mask_connect, \
            edge_maps, extend_context_ccs, extend_edge_ccs, extend_erode_context_ccs = \
                context_and_holes(input_mesh, new_edge_ccs, params, specific_edge_id, specific_edge_loc, depth_feat_model, connect_points_ccs, inpaint_iter=1)
    ...
    str_faces = generate_face(input_mesh, info_on_pix, params)
    if params['save_ply'] is True:
...
def output_3d_photo(verts, colors, faces, Height, Width, hFov, vFov, tgt_poses, video_traj_types, ref_pose,
                    output_dir, ref_image, int_mtx, params, image, videos_poses, video_basename, original_H=None, original_W=None,
    ...
    # print("fov: " + str(fov))
    init_factor = 1
    if params.get('anti_flickering') is True:
        init_factor = 3
    ...
    for video_pose, video_traj_type in zip(videos_poses, video_traj_types):
        ...
        for tp_id, tp in enumerate(video_pose):
            ...
            if any(np.array(params['crop_border']) > 0.0):
                H_c, W_c, _ = img.shape
                o_t = int(H_c * params['crop_border'][0])
                o_l = int(W_c * params['crop_border'][1])
                o_b = int(H_c * params['crop_border'][2])
                o_r = int(W_c * params['crop_border'][3])
        ...
        clip = ImageSequenceClip(stereos, fps=params['fps'])
        if isinstance(video_basename, list):
            video_basename = video_basename[0]
        clip.write_videofile(os.path.join(output_dir, video_basename + '_' + video_traj_type + '.mp4'), fps=params['fps'])
