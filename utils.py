...
def clean_far_edge_new(input_edge, end_depth_maps, mask, context, global_mesh, info_on_pix, self_edge, inpaint_id, params):
    mesh = netx.Graph()
    ...
    while end_idx < len(end_pts):
        ...
        if len(end_pt) == 2:
            ...
            if np.count_nonzero(re_npath_map1 * re_npath_map2 * mask) / \
                ...
                if npath_len > params['depth_edge_dilate_2'] * 2:
                    npath_len = npath_len - (params['depth_edge_dilate_2'] * 1)
        ...
        if len(end_pt) == 1:
            ...
            fpath = []
            if global_mesh.nodes[ends[0]].get('far') is not None:
...
def plan_path_e2e(mesh, cc, end_pts, global_mesh, input_edge, mask, valid_map, inpaint_id, npath_map=None, fpath_map=None):
    ...
    fpath = []
    if global_mesh.nodes[ends_1].get('far') is not None:
...
def get_MiDaS_samples(params, specific=None, aft_certain=None):
    image_folder = params['src_dir']
    depth_folder = params['outputs_dir']
    lines = [os.path.splitext(os.path.basename(xx))[0] for xx in glob.glob(os.path.join(image_folder, '*' + params['img_format']))]
    samples = []
    generic_pose = np.eye(4)
    tgt_pose = [[generic_pose * 1]]
    tgts_poses = []
    for traj_idx in range(len(params['traj_types'])):
        tgt_poses = []
        sx, sy, sz = path_planning(params['num_frames'], params['x_shift_range'][traj_idx], params['y_shift_range'][traj_idx],
                                   params['z_shift_range'][traj_idx], path_type=params['traj_types'][traj_idx])
    ...
    for seq_dir in lines:
        ...
        samples.append({})
        sdict = samples[-1]      
        sdict['depth_fi'] = os.path.join(depth_folder, seq_dir + params['depth_format'])
        sdict['ref_img_fi'] = os.path.join(image_folder, seq_dir + params['img_format'])
        ...
        sdict['video_postfix'] = params['video_postfix']
...
def refine_color_around_edge(mesh, info_on_pix, edge_ccs, params, spdb=False):
    H, W = mesh.graph['H'], mesh.graph['W']
...
def refine_depth_around_edge(mask_depth, far_edge, uncleaned_far_edge, near_edge, mask, all_depth, params):
    ...
    for i in range(params['depth_edge_dilate_2']):
...
def vis_depth_edge_connectivity(depth, params):
    disp = 1./depth
    u_diff = (disp[1:, :] - disp[:-1, :])[:-1, 1:-1]
    b_diff = (disp[:-1, :] - disp[1:, :])[1:, 1:-1]
    l_diff = (disp[:, 1:] - disp[:, :-1])[1:-1, :-1]
    r_diff = (disp[:, :-1] - disp[:, 1:])[1:-1, 1:]
    u_over = (np.abs(u_diff) > params['depth_threshold']).astype(np.float32)
    b_over = (np.abs(b_diff) > params['depth_threshold']).astype(np.float32)
    l_over = (np.abs(l_diff) > params['depth_threshold']).astype(np.float32)
    r_over = (np.abs(r_diff) > params['depth_threshold']).astype(np.float32)
